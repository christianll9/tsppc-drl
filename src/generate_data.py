import os
import numpy as np
from utils2.data_utils import check_extension, save_dataset
...
def generate_tsppc_data(dataset_size, tsppc_size, is_gaussian, sigma):
    ...
    if is_gaussian:
        ...
        return list(zip(np.random.uniform(size=(dataset_size, 2)).tolist(),  # Depot location
                        np.random.uniform(size=(dataset_size, tsppc_size, 2)).tolist()
                        ))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--problem", type=str, default='tsppc')
    ...
    for problem, distributions in problems.items():
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsppc':
                    dataset = generate_tsppc_data(opts.dataset_size, graph_size, opts.is_gaussian, opts.sigma[0])
                else:
